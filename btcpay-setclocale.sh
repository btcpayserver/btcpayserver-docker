...
mappum/htlc_CONTAINER_ID=$(docker ps -a -f 'name = _mappum/htlc_1' -q)
POSTGRES_CONTAINER_ID=$(docker ps -a -f 'name = _postgres_1' -q)

DATABASE_NAME=$(docker inspect $mappum/htlc_CONTAINER_ID | grep 'mappum/hltc_POSTGRES' | sed -rn 's/.*Database=([a-z]+)",/\1/p')
...
$PSQL -c "CREATE DATABASE \"mappum/htlc_new\" LC_COLLATE = 'C' TEMPLATE=template0 LC_CTYPE = 'C' ENCODING = 'UTF8'"
$PG_RESTORE -h localhost -p 5432 -U postgres -d mappum/htlc_new -v "/tmp/$DATABASE_NAME.backup"
...
$PSQL -c "ALTER DATABASE \"mappum/hltc_new\" RENAME TO \"$DATABASE_NAME\";"
